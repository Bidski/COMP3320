...
int main(void)
{
    // Get start time so we can animate the example.
    ...
    // Find the overrideColour uniformr.
    GLint uniColour = glGetUniformLocation(shaderProgram, "overrideColour");
    ...
    while(glfwWindowShouldClose(window) == GL_FALSE)
    {
        ...
        float timeDiff = std::chrono::duration_cast<std::chrono::duration<float>>(timeNow - startTime).count();

        glm::mat4 model;
        model = glm::rotate(model, timeDiff * glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0f));

        // Upload the transformations to the shader program.
        ...
        glEnable(GL_STENCIL_TEST);
            ...
            glDepthMask(GL_FALSE);                          // Don't draw to depth buffer.
            glClear(GL_STENCIL_BUFFER_BIT);                 // Clear the stencil buffer.

            glDrawArrays(GL_TRIANGLES, 36, 6);

            // Draw cube reflection.
            glStencilFunc(GL_EQUAL, 1, 0xFF);               // Pass test if stencil is equal to 1.
            ...
            glUniform3f(uniColour, 0.3f, 0.3f, 0.3f);       // Darken the reflection.
            glDrawArrays(GL_TRIANGLES, 0, 36);
            glUniform3f(uniColour, 1.0f, 1.0f, 1.0f);
